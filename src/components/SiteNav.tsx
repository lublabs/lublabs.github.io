...
export default function SiteNav({
  ...
  const navLinkClass = (key: ExtendedNavKey) =>
    `text-base font-medium transition ${
      resolvedActive === key
        ? "text-[#1F1A14] border-b border-[#0F766E] pb-0.5"
        : "text-[#6F6557] hover:text-[#1F1A14]"
    }`;

  const mobileNavLinkClass = (key: ExtendedNavKey) =>
    `rounded-md px-3 py-2 text-sm font-medium transition ${
      resolvedActive === key
        ? "bg-[#0F766E1A] text-[#1F1A14]"
        : "text-[#6F6557] hover:bg-black/5 hover:text-[#1F1A14]"
    }`;
  ...
  return (
    <nav data-site-nav className="site-nav fixed inset-x-0 top-0 z-50 border-b px-[5%] py-3 backdrop-blur-md md:py-4">
      <div className="mx-auto flex w-full max-w-[1400px] items-center justify-between gap-3 md:gap-6">
        <a href="/" className="font-display text-xl font-extrabold tracking-[-0.02em] text-[#1F1A14] sm:text-2xl md:text-[2rem]">
        ...
        <details className="relative md:hidden">
          <summary className="nav-summary flex list-none items-center gap-2 rounded-md border border-black/15 px-3 py-2 text-sm font-semibold text-[#1F1A14] transition hover:border-black/30 hover:bg-black/5">
            <span>Menu</span>
          ...
          <div className="absolute right-0 top-[calc(100%+10px)] w-[min(86vw,280px)] rounded-xl border border-black/10 bg-[#FFFDF8F2] p-2 shadow-[0_18px_44px_rgba(33,22,12,0.12)] backdrop-blur-md">
